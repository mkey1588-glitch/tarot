...
class Chart:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "birth_local": self.birth_local.isoformat(),
...
def compute_chart(birth_local: datetime,
                  options: ChartOptions | None = None) -> Chart:
    ...
    if options.apply_local_mean_time:
    ...
    gap = abs((birth_utc - lichun_this).total_seconds()) / 60.0
    if gap <= options.boundary_warning_minutes:
        warnings.append(
            f"Born within {gap:.0f} min of 立春 {lichun_this.astimezone(JST):%Y-%m-%d %H:%M} JST; "
            "the YEAR pillar is uncertain at this precision. Route to a practitioner."
        )
    ...
    for name, instant, branch in boundaries:
        ...
        gap = abs((birth_utc - instant).total_seconds()) / 60.0
        if gap <= options.boundary_warning_minutes and name != "立春":
            warnings.append(
                f"Born within {gap:.0f} min of {name} "
                f"{instant.astimezone(JST):%Y-%m-%d %H:%M} JST; "
                "the MONTH pillar is uncertain at this precision. Route to a practitioner."
            )
    ...
    if options.day_changes_at_2300 and effective.hour == 23:
